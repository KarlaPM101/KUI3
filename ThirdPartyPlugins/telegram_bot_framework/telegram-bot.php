...
Description: Daaaaaaaaaaaaaaaaaaaa
Version:      1
Author:       Original: Marco Milesi; Modified: Karla Pérez
...
function telegram_sendmessage( $chat_id, $text, $reply_markup = false, $disable_web_page_preview = false, $disable_notification = false ) {
    ...
    if(is_numeric($text)) {$image = wp_get_attachment_image_src($text, 'large', true );telegram_sendphoto($chat_id,'', $image[0]);return;}
    ...
    $Endpoint_Get = file_get_contents($url . '?' . http_build_query($data));
...
function telegram_sendphoto($chat_id, $caption, $photo) {
    ...
    $url = 'https://api.telegram.org/bot' . telegram_option('token') . '/sendPhoto?chat_id='.$chat_id.'&photo='.$photo.'&caption='.$caption;
    $data = compact('chat_id', 'photo', 'reply_markup');
...
function telegram_sendfile($chat_id, $caption, $document) {
    ...
    $url = 'https://api.telegram.org/bot' . telegram_option('token') . '/sendDocument?chat_id='.$chat_id.'&document='.$document;
